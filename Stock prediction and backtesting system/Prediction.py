...
def A_prediction(codes,startdate,enddate, datadate):
    ...
    for i in range(len(codes)):
        ...
        train_data, test_data = data[pd.to_datetime(data.index)<pd.to_datetime(startdate)], data[pd.to_datetime(data.index) >=pd.to_datetime(startdate)]
        # 定阶
    ...
    pred_close=pred_close.set_index(data_example[pd.to_datetime(data_example.index)>=pd.to_datetime(startdate)].index)
