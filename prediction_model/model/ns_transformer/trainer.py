...
from prediction_model.model.ns_transformer.dataset import StockDataset, TestDataset
from prediction_model.model.ns_transformer.ns_transformer import NSTransformer
...
def predict(datas: Optional[pd.DataFrame], start_date, field):
    config = Config()
    test_dataset = TestDataset(datas, start_date, field)
    ...
    result = result[:, :, test_dataset.pred_idx]
    result = np.concatenate((result[:, 0], np.array([result[-1, 1]])), axis=0)
    return result.tolist(), test_dataset.labels, test_dataset.dates
